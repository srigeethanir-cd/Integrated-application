...
import { Download, FileText, FileSpreadsheet, File, Loader2, AlertTriangle, Cloud, ArrowLeft } from 'lucide-react';
...
export default function ExportPage({ projectId: propProjectId, onNavigate }: { projectId?: string, onNavigate?: (tab: string) => void } = {}) {
      ...
      <div className="p-6 md:p-10 bg-muted/10 min-h-screen">
        <div className="max-w-4xl mx-auto bg-card border border-border shadow-sm p-8 md:p-12 rounded-xl text-foreground font-sans">
          ...
          <div className="mt-12 pt-8 border-t border-border flex items-center justify-between">
            {useWorkspaceStore.getState().workspaces.find(w => w.id === projectId)?.status === 'completed' ? (
              <Button
                variant="ghost"
                onClick={() => onNavigate ? onNavigate('Dashboard') : router.push(`/dashboard`)}
                className="text-muted-foreground hover:text-foreground hover:bg-accent px-4 h-10 rounded-lg text-sm flex items-center gap-2"
              >
