...
export default function StoryBoardPage({ projectId: propProjectId, onNavigate }: { projectId?: string; onNavigate?: (tab: string) => void } = {}) {
...
function StoryCard({
  ...
  const handleRestoreStory = async (versionToRestore: number) => {
    try {
      ...
      const updated = await api.getStory(projectId, story.id);
      if (updated) {
        onUpdate(updated);
      } else {
