...
export default function EpicReviewPage({ projectId: propProjectId, onNavigate }: { projectId?: string; onNavigate?: (tab: string) => void } = {}) {
  ...
  const handleRestoreEpic = async (epicId: string, versionToRestore: number) => {
    try {
      ...
      const res = await api.getEpics(projectId);
      const updated = res.find(e => e.id === epicId);
      if (updated) {
        setEpics(prev => prev.map(e => e.id === epicId ? updated : e));
      } else {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-background text-foreground relative">
      ...
      <div className="sticky bottom-0 left-0 right-0 p-6 bg-card/80 backdrop-blur-md border-t border-border flex items-center justify-between z-10">
        ...
        <Button
          disabled={!canProceed}
          onClick={workflowStatus === 'OUTLINE_REVIEW_REQUIRED' ? handleApproveOutline : () => onNavigate ? onNavigate('Story Board') : router.push(`/projects/${projectId}/stories`)}
          size="lg"
