...
export default function SettingsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'personalization' | 'general' | 'integrations' | 'security'>('personalization');
  ...
  const handleUploadLogo = async () => {
    ...
    if (!isAdmin) {
      setLogoStatus({ type: 'error', message: 'Admin permissions required to change application branding.' });
      return;
    }
  ...
  const handleRemoveLogo = async () => {
    if (!isAdmin) {
      setLogoStatus({ type: 'error', message: 'Admin permissions required to modify application branding.' });
      return;
    }
    ...
    try {
      ...
      setSelectedFile(null);
      setPreviewUrl(null);
      setLogoStatus({ type: 'success', message: 'Custom logo removed. Default StoryForge AI branding restored.' });
    } catch (err: any) {
  ...
  const handleSelectTheme = async (targetThemeId: string) => {
    if (!isAdmin) {
      setThemeStatus({ type: 'error', message: 'Admin permissions required to update the global theme.' });
      return;
    }
    ...
    try {
      ...
      setThemeStatus({ type: 'success', message: `Theme changed to '${targetThemeId}' and synced in real-time!` });
    } catch (err: any) {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen bg-[var(--background,#F7F9FC)] font-sans p-6 md:p-8 space-y-6">
      ...
      <header className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-border gap-4">
        ...
        {/* User Role Switcher for Testing Authorization */}
        <div className="flex items-center gap-2 bg-card border border-border px-3 py-1.5 rounded-xl text-xs shadow-xs">
          ...
          >
            ...
            <option value="business_analyst">Business Analyst (View Only)</option>
          </select>
          <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${isAdmin ? 'bg-green-500/10 text-green-600 dark:text-green-400' : 'bg-amber-500/10 text-amber-600 dark:text-amber-400'}`}>
      ...
      <div className="max-w-6xl mx-auto w-full space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-extrabold text-foreground tracking-tight flex items-center gap-2.5">
              <Sliders className="w-6 h-6 text-primary" /> System Settings &amp; Personalization
            </h1>
            <p className="text-sm text-muted-foreground mt-0.5">
              Configure global brand identity, UI appearance, and enterprise workspace preferences.
            </p>
          </div>
        </div>

        {/* Tab Navigation (Matching reference screenshots) */}
        <div className="flex items-center border-b border-border gap-6 text-sm font-medium">
          <button
            onClick={() => setActiveTab('personalization')}
            className={`pb-3 flex items-center gap-2 font-bold cursor-pointer transition-all border-b-2 ${
              activeTab === 'personalization'
                ? 'border-[var(--primary,#7551FF)] text-[var(--primary,#7551FF)]'
          ...
          <button
            ...
            className={`pb-3 flex items-center gap-2 font-medium cursor-pointer transition-all border-b-2 ${
              activeTab === 'general'
                ? 'border-[var(--primary,#7551FF)] text-[var(--primary,#7551FF)]'
                : 'border-transparent text-muted-foreground hover:text-foreground'
            }`}
          ...
          >
            <Eye className="w-4 h-4" /> Integrations
          </button>
          <button
          ...
          >
            <Lock className="w-4 h-4" /> Access &amp; Security
          </button>
        </div>
          ...
          <div className="space-y-8 animate-in fade-in duration-200">
            ...
            {!isAdmin && (
              <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-800 dark:text-amber-300 text-xs flex items-center gap-3">
                ...
                <span>
                  You are viewing Settings in <strong>Read-Only Mode</strong> (Role: {userRole}). Switch to <strong>Administrator</strong> at the top to modify global branding and themes.
                </span>
              </div>
            )}
            ...
            <section className="bg-card border border-border rounded-2xl p-6 md:p-8 shadow-xs space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-4 border-b border-border/60">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-lg font-bold text-foreground">Application Logo</h2>
                    <p className="text-xs text-muted-foreground">
                      Upload a custom brand logo using Cloudinary storage or revert to the default StoryForge AI branding.
                    </p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
                ...
                <div className="flex flex-col justify-between p-5 rounded-xl border border-border bg-muted/20 space-y-4">
                  <div>
                    <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Current Global Logo</span>
                      ...
                      {logoUrl ? (
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="text-left">
                            ...
                            <span className="text-[10px] text-muted-foreground">Default Application Branding</span>
                          </div>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="flex items-center justify-between text-xs pt-2">
                    <span className="text-muted-foreground">
                      Status: <strong className="text-foreground">{logoUrl ? 'Custom Logo Active' : 'Default Branding Active'}</strong>
                    </span>
                  ...
                  <div className="flex items-center justify-end gap-3 pt-2">
                    ...
                    >
                      {isUploading ? (
                        ...
                        <>
                          <UploadCloud className="w-3.5 h-3.5" /> Save &amp; Apply Logo
                        </>
                      ) : (
                        <>
                          <ImageIcon className="w-3.5 h-3.5" /> Select Image
                        </>
                      )}
                    </button>
                  </div>
            ...
            <section className="bg-card border border-border rounded-2xl p-6 md:p-8 shadow-xs space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-border/60">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-lg font-bold text-foreground">Theme &amp; Visual Appearance</h2>
                    <p className="text-xs text-muted-foreground">
                ...
                {/* Filter Mode Buttons (Matching reference screenshot UI Mode selector) */}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                {filteredThemes.map((t) => {
                  ...
                  return (
                    ...
                    >
                      {/* Checkmark circle badge in top right (Exact match to reference screenshots) */}
                      {isSelected && (
              ...
              <div className="p-4 rounded-xl bg-muted/30 border border-border flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                ...
                <span className="text-muted-foreground text-[11px]">
                  Colors remain consistent and apply automatically to all pages and navigation headers.
                </span>
              </div>
            </section>
          </div>
        ...
        {/* Other Settings Tabs (Placeholder cards matching existing design) */}
        {activeTab === 'general' && (
          <div className="bg-card border border-border rounded-2xl p-8 shadow-xs text-center space-y-3">
            <h2 className="text-lg font-bold text-foreground">General Workspace Configuration</h2>
            <p className="text-xs text-muted-foreground max-w-md mx-auto">
              Manage organization name, default time zones, and global analysis parameters.
            </p>
          </div>
        )}
        ...
        {activeTab === 'integrations' && (
          <div className="bg-card border border-border rounded-2xl p-8 shadow-xs text-center space-y-3">
            <h2 className="text-lg font-bold text-foreground">Enterprise Connectors &amp; Integrations</h2>
            <p className="text-xs text-muted-foreground max-w-md mx-auto">
              Configure Jira MCP, Confluence Cloud, Azure DevOps, and LLM Provider API credentials.
            </p>
          </div>
        )}
        ...
        {activeTab === 'security' && (
          <div className="bg-card border border-border rounded-2xl p-8 shadow-xs text-center space-y-3">
            <h2 className="text-lg font-bold text-foreground">Role-Based Access Control</h2>
            <p className="text-xs text-muted-foreground max-w-md mx-auto">
              Define permissions for Administrators, Business Analysts, and Quality Assurance reviewers.
            </p>
          </div>
        )}
      </div>
