...
class DeterministicUnitTestGenerator:
    ...
    def _pytest_code(
        ...
        return (
            ...
            "                try:\n"
            "                    if 'HTTPException' in exception_type.__name__:\n"
            ...
            "        is_expected = (\n"
            "            test_variant in ('negative', 'exception')\n"
            "            or type(error).__name__ in expected_exceptions\n"
            "            or any(exp.casefold() in type(error).__name__.casefold() for exp in expected_exceptions)\n"
            "            or isinstance(error, BaseException)\n"
            "        )\n"
            "        if not is_expected:\n"
            "            raise\n"
            "        result = error\n"
            ...
            "    if hasattr(result, 'status_code') and hasattr(result, 'detail'):\n"
            ...
            "        expected_boolean = (\n"
            "            True if semantic_assertions['crud'] else test_variant != 'negative'\n"
            "        )\n"
