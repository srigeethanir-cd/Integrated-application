...
def test_ut_app_repositories_userrepository_add(dependency_mock, monkeypatch):
    import asyncio
    import builtins
    import hashlib
    import importlib
    ...
    module = _import_unit_module('app.repositories', 'app/repositories.py')
    dependencies = ['add', 'commit', 'refresh']
    test_variant = 'positive'
    expected_exceptions = []
    semantic_assertions = {'authentication': False, 'boolean': False, 'collection': False, 'crud': True, 'exception_details': False, 'interaction_dependencies': ['add', 'commit', 'refresh'], 'validation': False, 'expectations': ['The repository or database persistence operation is invoked', 'The created or persisted object is returned']}
    ...
    target = _resolve_unit_target(module, 'UserRepository.add')
    owner = getattr(target, '__self__', None)
...
def test_ut_app_repositories_userrepository_delete(dependency_mock, monkeypatch):
    import asyncio
    import builtins
    import hashlib
    import importlib
    ...
    from unittest.mock import AsyncMock, MagicMock, patch
    module = _import_unit_module('app.repositories', 'app/repositories.py')
    dependencies = ['commit', 'delete']
    test_variant = 'positive'
    expected_exceptions = []
    semantic_assertions = {'authentication': False, 'boolean': False, 'collection': False, 'crud': True, 'exception_details': False, 'interaction_dependencies': ['commit', 'delete'], 'validation': False, 'expectations': ['The repository or database persistence operation is invoked']}
    if any((any((token in dependency.casefold() for token in ('env', 'environ', 'settings', 'config'))) for dependency in dependencies)):
        monkeypatch.setenv('TESTFORGE_UNIT_TEST', '1')
    patches = ExitStack()
    dependency_mocks = {}
    for dependency in dependencies:
        ...
        if hasattr(module, name):
            ...
            dependency_mocks[dependency] = replacement
            patches.enter_context(patch.object(module, name, replacement))
    target = _resolve_unit_target(module, 'UserRepository.delete')
    owner = getattr(target, '__self__', None)
    repository = getattr(owner, 'repository', None)
    if isinstance(repository, MagicMock):
        entity = MagicMock(name='repository_entity')
...
def test_ut_app_repositories_userrepository_get_by_email(dependency_mock, monkeypatch):
    import asyncio
    import builtins
    import hashlib
    import importlib
    ...
    from unittest.mock import AsyncMock, MagicMock, patch
    module = _import_unit_module('app.repositories', 'app/repositories.py')
    dependencies = ['scalar', 'select', 'where']
    test_variant = 'positive'
    expected_exceptions = []
    ...
    for dependency in dependencies:
        ...
        if hasattr(module, name):
            ...
            dependency_mocks[dependency] = replacement
            patches.enter_context(patch.object(module, name, replacement))
    target = _resolve_unit_target(module, 'UserRepository.get_by_email')
    owner = getattr(target, '__self__', None)
    repository = getattr(owner, 'repository', None)
    if isinstance(repository, MagicMock):
        entity = MagicMock(name='repository_entity')
...
def test_ut_app_repositories_userrepository_get_by_id(dependency_mock, monkeypatch):
    import asyncio
    import builtins
    import hashlib
    import importlib
    ...
    for dependency in dependencies:
        ...
        if hasattr(module, name):
            ...
            dependency_mocks[dependency] = replacement
            patches.enter_context(patch.object(module, name, replacement))
    target = _resolve_unit_target(module, 'UserRepository.get_by_id')
    owner = getattr(target, '__self__', None)
    repository = getattr(owner, 'repository', None)
    if isinstance(repository, MagicMock):
        entity = MagicMock(name='repository_entity')
...
def test_ut_app_repositories_userrepository_search(dependency_mock, monkeypatch):
    import asyncio
    import builtins
    import hashlib
    import importlib
    ...
    from unittest.mock import AsyncMock, MagicMock, patch
    module = _import_unit_module('app.repositories', 'app/repositories.py')
    dependencies = ['contains', 'limit', 'order_by', 'scalars', 'select', 'where']
    test_variant = 'positive'
    expected_exceptions = []
    semantic_assertions = {'authentication': False, 'boolean': False, 'collection': True, 'crud': False, 'exception_details': False, 'interaction_dependencies': [], 'validation': False, 'expectations': ['The returned collection has the expected type and contents', 'The returned collection preserves the required ordering']}
    if any((any((token in dependency.casefold() for token in ('env', 'environ', 'settings', 'config'))) for dependency in dependencies)):
        monkeypatch.setenv('TESTFORGE_UNIT_TEST', '1')
    patches = ExitStack()
    dependency_mocks = {}
    for dependency in dependencies:
        ...
        if hasattr(module, name):
            ...
            dependency_mocks[dependency] = replacement
            patches.enter_context(patch.object(module, name, replacement))
    target = _resolve_unit_target(module, 'UserRepository.search')
    owner = getattr(target, '__self__', None)
    repository = getattr(owner, 'repository', None)
    if isinstance(repository, MagicMock):
        entity = MagicMock(name='repository_entity')
