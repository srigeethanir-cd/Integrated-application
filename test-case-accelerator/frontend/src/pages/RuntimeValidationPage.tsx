...
function failureReason(result: RuntimeExecutionResult) {
  ...
  const failureText = result.assertion_failure ?? ''
  if (failureText) {
    const lines = failureText.split('\n').map(l => l.trim()).filter(Boolean)
    const assertionLine = lines.find(l => l.startsWith('E ') || l.startsWith('AssertionError:') || l.includes('Expected') || l.includes('Error'))
    if (assertionLine) {
      const clean = assertionLine.replace(/^E\s+/, '').replace(/^AssertionError:\s*/i, '').trim()
      if (clean) return clean
    }
    const firstLine = lines[0]?.replace(/^AssertionError:\s*/i, '').trim()
  ...
  for (const name of ['TimeoutError', 'Timeout', 'AttributeError', 'ImportError', 'ModuleNotFoundError', 'TypeError', 'ValueError', 'KeyError']) {
    if (text.includes(name)) return name
  }
  return failureText.trim() || 'Runtime Failure'
}
...
export function RuntimeValidationPage() {
  ...
  const isTargetConnected = validateBaseUrl(baseUrl) && (!run || run.status !== 'failed')
  ...
  return <div className="page runtime-runner-page">
    ...
    <section className="runner-target"><div><span className={`connection-dot ${isTargetConnected ? 'connected' : ''}`} /><span><small>Target application</small><strong>{isTargetConnected ? 'Connected' : 'Disconnected'}</strong></span></div><label htmlFor="runtime-base-url"><span>Base URL</span><input id="runtime-base-url" type="url" value={baseUrl} onChange={(event) => setBaseUrl(event.target.value)} disabled={busy} /></label><Button onClick={() => void start()} disabled={busy || !understandingRunId}>{busy ? <><RefreshCw size={15} /> Validation running</> : <><RotateCcw size={15} /> {report ? 'Run Validation Again' : 'Run Runtime Validation'}</>}</Button></section>
