...
export const PromptManagement: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const perfRes = await fetch('/api/v1/prompt-templates/performance');
  ...
  const fetchVersions = async (templateId: string) => {
    try {
      const res = await fetch(`/api/v1/prompt-templates/${templateId}/versions`);
      const data = await res.json();
      if (data.success) {
  ...
  const handleUpdatePrompt = async () => {
    ...
    try {
      const res = await fetch(`/api/v1/prompt-templates/${selectedTemplate.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleApprovePrompt = async (decision: 'Approved' | 'Rejected') => {
    ...
    try {
      const res = await fetch(`/api/v1/prompt-templates/${selectedTemplate.id}/approve`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleRollback = async () => {
    ...
    try {
      const res = await fetch(`/api/v1/prompt-templates/${selectedTemplate.id}/rollback`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (data.success) {
        ...
        fetchData();
        fetchVersions(selectedTemplate.id);
        alert(`Successfully rolled back to version ${res.data.rolled_back_to}`);
      }
    } catch (err) {
