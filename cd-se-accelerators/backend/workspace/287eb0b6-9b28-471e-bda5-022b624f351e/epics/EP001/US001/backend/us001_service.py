...
# Define dependency to get database session
def get_db():
    db = SessionLocal()
    try:
        yield db
...
# Define endpoint to register user
@app.post("/register", response_model=UserRegistrationResponse)
def register_user(user_registration_request: UserRegistrationRequest, db = Depends(get_db)):
