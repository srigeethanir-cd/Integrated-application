...
const App: React.FC = () => {
  ...
  return (
    <div className="h-screen w-screen flex flex-col items-center justify-center">
      <header className="h-16 w-full bg-primary text-white flex items-center justify-center">
        <h1 className="text-2xl font-bold">User Stories</h1>
      </header>
      <main className="h-full w-full flex flex-col items-center justify-center">
        <div className="w-full h-full overflow-y-scroll">
          {userStories.map((story) => (
            <div key={story.id} className="p-4 border-b border-gray-300 cursor-pointer" onClick={() => handleUserStoryClick(story)}>
            ...
            <div className="bg-white p-4 rounded-lg shadow-lg">
              ...
              <ul>
                {userStory.acceptance_criteria.map((criterion) => (
                  <li key={criterion} className="text-gray-600">{criterion}</li>
                ))}
              </ul>
            </div>
          </div>
      ...
      <footer className="h-16 w-full bg-primary text-white flex items-center justify-center">
