  ...
  return (
    <div className="h-screen w-screen flex flex-col">
      <header className="h-16 bg-primary text-white p-4 flex justify-between">
        ...
        <button className="bg-secondary text-white p-2 rounded">Create New Story</button>
      </header>
      <main className="flex-1 overflow-y-scroll p-4">
        <ul>
          {userStories.map((story) => (
            <li key={story.id} className="mb-4">
              <button className="bg-primary text-white p-2 rounded" onClick={() => handleUserStoryClick(story)}>
                {story.title}
              </button>
            </li>
          ))}
      ...
      <footer className="h-12 bg-primary text-white p-4 flex justify-between">
