...
<body class="bg-slate-950 text-slate-100 min-h-screen p-8 flex flex-col justify-between">
  ...
  <script>
    ...
    def _extract_story_domain_metadata(story: Dict[str, Any], story_key: str, epic_key: str) -> Dict[str, Any]:
        """Dynamically extract rich domain terms, components, tables, and actions from story metadata."""
        import re
        ...
        words = [w.lower() for w in clean_title.split() if w.lower() not in ["as", "a", "user", "i", "want", "to", "so", "that", "the", "system", "can", "and", "or", "in", "on", "for"]]
        ...
        elif any(k in combined for k in ["profile", "account", "user info"]):
            module_name = "user_profile"
            comp_name = "UserProfile"
            tbl_name = "tbl_user_profiles"
            service_name = "UserProfileService"
            router_name = "user_profile_router"
            primary_action = "update_user_profile"
