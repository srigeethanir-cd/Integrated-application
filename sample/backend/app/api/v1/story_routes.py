...
def _resolve_story(story_id_str: str, db: Session):
    ...
    try:
        from app.api.v1.workspace_explorer_routes import get_story_by_id
...
def get_story_versions(story_id: str, db: Session = Depends(get_db)):
    ...
    for h in history:
        versions_data.append({
            "version": str(h.version) if h.version is not None else "1.0",
            "user": h.user or "Business Analyst",
            "action": h.action or "REVIEW",
            "location_type": h.action or "Audit Entry",
            "new_state": h.new_state or getattr(story, "approval_status", "APPROVED"),
            "comments": h.comments or f"Story state updated to {h.new_state}",
            "refinement_prompt": ref_map.get(h.version, None),
            "created_at": h.timestamp.strftime("%Y-%m-%d %H:%M:%S") if h.timestamp else "2026-08-25 12:00:00",
            "timestamp": h.timestamp.isoformat() if h.timestamp else None
        })
    
    # If no history entries exist yet, construct base lifecycle entries
    if not versions_data:
        versions_data = [
            {
                "version": "1.0",
                "user": "Agent 2 Code Generator",
                "action": "CODE_SYNTHESIS",
                "location_type": "Code Generation",
                "new_state": "GENERATED",
                "comments": f"Synthesized React TSX UI component, FastAPI router, SQLAlchemy schema, and Pytest suite for {story.story_title}.",
                "refinement_prompt": None,
                "created_at": "2026-08-25 12:00:00",
                "timestamp": "2026-08-25T12:00:00Z"
            }
        ]
        if getattr(story, "approval_status", "").upper() == "APPROVED":
            versions_data.insert(0, {
                "version": "1.0",
                "user": "Product Owner / Business Analyst",
                "action": "STORY_ACCEPTED",
                "location_type": "Approval Gate",
                "new_state": "APPROVED",
                "comments": f"Story {story.story_key or story_id} implementation reviewed and approved for integration.",
                "refinement_prompt": None,
                "created_at": "2026-08-25 12:45:00",
