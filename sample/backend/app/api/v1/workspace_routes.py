...
ALLOWED_ROOT_JSONS = {"storyexecutionsummary.json", "metadata.json", "generated_files.json"}
...
def _tree(directory: Path, root: Path, validation_status: Optional[str] = None) -> Dict[str, Any]:
    ...
    for item in sorted(directory.iterdir(), key=lambda path: (not path.is_dir(), path.name.lower())):
        ...
        # If at root level of story sandbox
        if is_root:
            if item.is_dir():
                if item_name_lower not in ALLOWED_TOP_DIRS:
                    continue
            else:
                if item_name_lower not in ALLOWED_ROOT_JSONS:
                    continue
        else:
            # Inside frontend/ or backend/ subtrees
            if item.is_dir():
                if item_name_lower in {"__pycache__", "node_modules", ".git", ".pytest_cache"}:
                    continue
            else:
                # Exclude unwanted internal JSON files inside frontend/backend
                if item.suffix.lower() == ".json" and item_name_lower not in ALLOWED_ROOT_JSONS and item_name_lower != "package.json":
                    continue
        ...
        if not item.is_dir():
            ...
            sub_tree = _tree(item, root, validation_status)
            node["children"] = sub_tree.get("children", [])
